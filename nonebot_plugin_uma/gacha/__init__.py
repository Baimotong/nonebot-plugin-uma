...
async def handle_one_uma(event: GroupMessageEvent):
    ...
    if not lmt.check(uid):
        await gacha_one_uma.finish(f"马娘抽卡功能冷却中(剩余 {int(lmt.left_time(uid)) + 1}秒)", at_sender=True)
    if not single_limit.check(uid):
        await gacha_one_uma.finish(SINGLE_EXCEED, at_sender=True)
    lmt.start_cd(uid)
    single_limit.increase(uid, 150)
    ok = await _ensure_data()
    if not ok:
        await gacha_one_uma.finish("卡池数据尚未准备好，请稍后再试")
    ...
    await gacha_one_uma.finish(msg)
...
async def handle_one_chart(event: GroupMessageEvent):
    uid = str(event.user_id)
    if not lmt.check(uid):
        await gacha_one_chart.finish(f"马娘抽卡功能冷却中(剩余 {int(lmt.left_time(uid)) + 1}秒)", at_sender=True)
    if not single_limit.check(uid):
        await gacha_one_chart.finish(SINGLE_EXCEED, at_sender=True)
    lmt.start_cd(uid)
    single_limit.increase(uid, 150)
    ok = await _ensure_data()
    if not ok:
        await gacha_one_chart.finish("卡池数据尚未准备好，请稍后再试")
    gid = str(event.group_id)
    server, pool_id = get_pool(_gacha_dir, gid)
    ...
    try:
        ...
        msg = "卡池数据还未自动更新完全，请耐心等待几小时后再尝试"
    await gacha_one_chart.finish(msg)
...
async def handle_ten_uma(event: GroupMessageEvent):
    uid = str(event.user_id)
    if not lmt.check(uid):
        await gacha_ten_uma.finish(f"马娘抽卡功能冷却中(剩余 {int(lmt.left_time(uid)) + 1}秒)", at_sender=True)
    if not single_limit.check(uid):
        await gacha_ten_uma.finish(SINGLE_EXCEED, at_sender=True)
    lmt.start_cd(uid)
    single_limit.increase(uid, 1500)
    ok = await _ensure_data()
    if not ok:
        await gacha_ten_uma.finish("卡池数据尚未准备好，请稍后再试")
    gid = str(event.group_id)
    server, pool_id = get_pool(_gacha_dir, gid)
    gacha = Gacha(pool_id, "uma", server, _gacha_dir)
    try:
        ...
        msg = "卡池数据还未自动更新完全，请耐心等待几小时后再尝试"
    await gacha_ten_uma.finish(msg)
...
async def handle_ten_chart(event: GroupMessageEvent):
    uid = str(event.user_id)
    if not lmt.check(uid):
        await gacha_ten_chart.finish(f"马娘抽卡功能冷却中(剩余 {int(lmt.left_time(uid)) + 1}秒)", at_sender=True)
    if not single_limit.check(uid):
        await gacha_ten_chart.finish(SINGLE_EXCEED, at_sender=True)
    lmt.start_cd(uid)
    single_limit.increase(uid, 1500)
    ok = await _ensure_data()
    if not ok:
        await gacha_ten_chart.finish("卡池数据尚未准备好，请稍后再试")
    gid = str(event.group_id)
    server, pool_id = get_pool(_gacha_dir, gid)
    gacha = Gacha(pool_id, "chart", server, _gacha_dir)
    try:
        ...
        msg = "卡池数据还未自动更新完全，请耐心等待几小时后再尝试"
    await gacha_ten_chart.finish(msg)
...
async def handle_tenjou_uma(event: GroupMessageEvent):
    uid = str(event.user_id)
    if not lmt.check(uid):
        await gacha_tenjou_uma.finish(f"马娘抽卡功能冷却中(剩余 {int(lmt.left_time(uid)) + 1}秒)", at_sender=True)
    if not tenjou_limit.check(uid):
        await gacha_tenjou_uma.finish(TENJOU_EXCEED, at_sender=True)
    lmt.start_cd(uid)
    tenjou_limit.increase(uid)
    ok = await _ensure_data()
    if not ok:
        await gacha_tenjou_uma.finish("卡池数据尚未准备好，请稍后再试")
    gid = str(event.group_id)
    server, pool_id = get_pool(_gacha_dir, gid)
    gacha = Gacha(pool_id, "uma", server, _gacha_dir)
    try:
        ...
        msg = "卡池数据还未自动更新完全，请耐心等待几小时后再尝试"
    await gacha_tenjou_uma.finish(msg)
...
async def handle_tenjou_chart(event: GroupMessageEvent):
    uid = str(event.user_id)
    if not lmt.check(uid):
        await gacha_tenjou_chart.finish(f"马娘抽卡功能冷却中(剩余 {int(lmt.left_time(uid)) + 1}秒)", at_sender=True)
    if not tenjou_limit.check(uid):
        await gacha_tenjou_chart.finish(TENJOU_EXCEED, at_sender=True)
    lmt.start_cd(uid)
    tenjou_limit.increase(uid)
    ok = await _ensure_data()
    if not ok:
        await gacha_tenjou_chart.finish("卡池数据尚未准备好，请稍后再试")
    gid = str(event.group_id)
    server, pool_id = get_pool(_gacha_dir, gid)
    gacha = Gacha(pool_id, "chart", server, _gacha_dir)
    try:
        ...
        msg = "卡池数据还未自动更新完全，请耐心等待几小时后再尝试"
    await gacha_tenjou_chart.finish(msg)
...
async def handle_full_chart(event: GroupMessageEvent):
    uid = str(event.user_id)
    if not lmt.check(uid):
        await gacha_full_chart.finish(f"马娘抽卡功能冷却中(剩余 {int(lmt.left_time(uid)) + 1}秒)", at_sender=True)
    if not full_limit.check(uid):
        await gacha_full_chart.finish(FULL_EXCEED, at_sender=True)
    lmt.start_cd(uid)
    full_limit.increase(uid)
    ok = await _ensure_data()
    if not ok:
        await gacha_full_chart.finish("卡池数据尚未准备好，请稍后再试")
    gid = str(event.group_id)
    server, pool_id = get_pool(_gacha_dir, gid)
    if pool_id == "00000000":
        await gacha_full_chart.finish("初始卡池00000000不支持该功能哦")
    gacha = Gacha(pool_id, "chart", server, _gacha_dir)
    try:
        ...
        msg = "卡池数据还未自动更新完全，请耐心等待几小时后再尝试"
    await gacha_full_chart.finish(msg)
